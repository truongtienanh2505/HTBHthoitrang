...
export const productCardHtml = (p) => {
    return `
        <a class="card" href="product.html?id=${p.id}" style="text-decoration:none; color:inherit; border: 1px solid #eaeaea; padding: 10px; border-radius: 8px; display: block; text-align: center;">
            <div class="card__img"><img alt="${p.name}" src="${p.image}" style="max-width: 100%; height: 200px; object-fit: cover; border-radius: 4px;"></div>
            <div class="card__body" style="margin-top: 10px;">
                <h3 class="card__title" style="font-size: 16px; margin-bottom: 5px;">${p.name}</h3>
                <div class="card__price"><div class="price" style="color: red; font-weight: bold;">${p.price} VNĐ</div></div>
            </div>
        </a>
    `;
};
